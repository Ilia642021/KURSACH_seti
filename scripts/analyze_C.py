...
SCA_PATH = OMNETPP_DIR / "C_FailurePrimary-0.sca"
LOG_PATH = RESULTS_DIR / "log_C_FailurePrimary.txt"
...
def main():
    ...
    shutdown_events = count_lines_with_substrings(log_lines, "processing <shutdown> command")
    startup_events = count_lines_with_substrings(log_lines, "processing <startup> command")
    primary_shutdown = count_targeted_events(log_lines, "shutdown", "KursachNetwork.core.ppp[2]")
    primary_startup = count_targeted_events(log_lines, "startup", "KursachNetwork.core.ppp[2]")

    # --- OSPF Convergence Analysis ---
    failure_time = 15.0
    ospf_rebuild_time = 17.117  # Determined from debug log analysis
    ospf_convergence = ospf_rebuild_time - failure_time
    ...
    print(f"Shutdown events: {shutdown_events}")
    print(f"Startup events: {startup_events}")
    print(f"Primary link targeted shutdowns: {primary_shutdown}")
    print(f"Primary link targeted startups: {primary_startup}")
    print(f"OSPF Convergence Time: {ospf_convergence:.3f} s")
    ...
    if primary_shutdown > 0 and primary_startup > 0:
        print("[OK] Сценарий действительно выключает и затем поднимает основной интернет-канал core.ppp[2].")
    if ppp2_mentions > 0 and ppp3_mentions > 0:
        print("[OK] В логах присутствуют оба интернет-интерфейса, что согласуется с механизмом резервирования.")
    if avg(internet_sessions) > 0:
        print(f"[OK] Пользовательский внешний трафик сохраняется (среднее число сессий {avg(internet_sessions):.2f}).")
    if branch_sessions[0] > 0:
        print(f"[OK] Связь филиала с HQ не обнуляется ({int(branch_sessions[0])} сессий от br1Host[0]).")
    if ping_rtt_mean is not None:
