...
*Рис. 2.3.1 - Подробная схема сети с указанием физической среды передачи*
...
*Рис. 2.8 - Сформированные файлы результатов (.vec, .sca) для всех сценариев в IDE*

### Отчет по событиям моделирования
В таблице 3.1 и последующем описании приведены ключевые этапы жизненного цикла модели во время симуляции.
...
- **OSPF Convergence:** Полное перестроение маршрутов за 2.117 секунды.
...
## 3.4 Анализ влияния резервного канала связи (индивидуальное задание)
...
### 3.4.1 Сравнение пропускной способности при активном резервном канале (сценарий B)
Наличие резервного канала в «горячем» режиме (Scenario B) не снижает производительность основного канала. График загрузки интерфейса `ppp2` в сценарии B идентичен сценарию A, что подтверждает корректную работу OSPF: при наличии пути с меньшей стоимостью (Cost 10 против 50), весь трафик направляется через основной линк.

### 3.4.2 Влияние резервного канала на задержку в штатном режиме
Сравнение сценариев A и B показывает, что наличие включенного резервного интерфейса не вносит дополнительных задержек в обработку пакетов.
...
### 3.4.3 Поведение TCP при переключении на резерв
При переключении на резервный канал в сценарии C наблюдается изменение интенсивности трафика (см. рис. 3.3.2). Это вызвано увеличением RTT в облаке InternetCloud (с 20 до 40 мс), что напрямую влияет на пропускную способность TCP-сессий из-за увеличения времени ожидания подтверждений (ACK).

### 3.4.4 Выводы по индивидуальному заданию
Резервный канал обеспечивает требуемую отказоустойчивость сети без потери пакетов в момент аварии. Время переключения (2.117 с) является приемлемым для работы бизнес-приложений. В штатном режиме наличие резервного канала не ухудшает производительность сети.
...
**Вывод:** Гистограмма демонстрирует изменение средней задержки в зависимости от условий эксплуатации сети. Для сценариев А и В показатель стабилен (12.554 мс). В сценарии масштабирования (D) задержка возрастает до 13.763 мс из-за роста объема трафика. Максимальное среднее значение зафиксировано в сценарии C (16.577 мс). Данный рост обусловлен кратковременной задержкой в период сходимости OSPF и более высоким собственным RTT резервного канала.
...
**Таблица 3.4 — Сводные результаты моделирования по сценариям**
...
| Ping RTT HQ (мс) | 0.841 | 0.841 | 0.843 | 0.845 |
| TCP Delay по HQ (мс) | 12.554 | 12.554 | 16.577 | 13.763 |
| Потери пакетов | 0% | 0% | 0% | 0% |
| Состояние OSPF | Full (Stable) | Full (Stable) | Exchange (Failover) | Full (Stable) |

*\* Примечание: Анализ сценария C показал, что благодаря использованию статических маршрутов в качестве запасных путей и быстрой сходимости OSPF, реальные потери пакетов отсутствуют (0%). Незначительное снижение интенсивности трафика в момент переключения вызвано кратковременным ожиданием TCP-подтверждений (ACK) при изменении маршрута.*
...
\[ L_{avg} = L_{req} + L_{rep} = 800 + 3200 = 4000 \text{ байт} = 32000 \text{ бит} \qquad (4.1) \]
...
\[ P_{srv} = \frac{N \times L_{avg}}{T_{interval}} \approx 4.1 \text{ Мбит/с} \qquad (4.2) \]
...
- **Поток филиалов (4 РМ):** \[ P_{br} = 4 \times 0.12 \text{ Мбит/с} \approx 0.5 \text{ Мбит/с} \qquad (4.3) \]
- **Суммарная нагрузка:** \[ P_{total} = P_{hq} + P_{br} \approx 4.55 \text{ Мбит/с} \qquad (4.4) \]

### 4.3. Расчет загрузки резервного канала
...
\[ \rho_{res} = \frac{P_{total}}{C} \times 100\% = \frac{4.55}{30} \times 100\% \approx 15.1\% \qquad (4.5) \]
...
### 4.4. Теоретическая оценка времени сходимости OSPF
...
\[ T_{conv} \leq \text{Dead\_interval} + T_{SPF} \qquad (4.6) \]
...
### 4.5. Расчет максимальной пропускной способности при масштабировании
Для оценки производительности при увеличении числа рабочих мест используется формула линейной аппроксимации:
\[ P_{max}(scale) = \frac{N_{new}}{N_{base}} \times P_{base} \qquad (4.7) \]
Где:
...
\[ P_{max} = \frac{43}{30} \times 4547.71 \approx 6518.3 \text{ Кбит/с} \qquad (4.8) \]
...
### 4.6. Коэффициент использования каналов
...
\[ \rho_{pri} = \frac{4.55}{30} \approx 0.151 \text{ (15.1%)} \qquad (4.9) \]

**Для сценария масштабирования (Scenario D, 43 РМ):**
\[ \rho_{scale} = \frac{5.88}{30} \approx 0.196 \text{ (19.6%)} \qquad (4.10) \]
...
# ТРЕБОВАНИЯ К ОБОРУДОВАНИЮ ПО УРОВНЯМ ИЕРАРХИИ
...
**Таблица 4.1 — Функциональные требования к оборудованию**
